...
class Neo4JStorage(BaseGraphStorage):
    def __init__(self, namespace, global_config, embedding_func):
        ...
        # For Neo4j Aura, always use "neo4j" as the database name
        ...
        # Simple connection test without database creation attempts
        with GraphDatabase.driver(
            ...
            try:
                with _sync_driver.session(database=self._DATABASE) as session:
                    session.run("MATCH (n) RETURN n LIMIT 0")
                    logger.info(f"Successfully connected to Neo4j Aura database at {URI}")
